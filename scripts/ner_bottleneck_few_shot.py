...
import LlmBase
import random
from ner_bottleneck import build_highlight_prompt
...
class FewShot(LlmBase.AbstractLLM):
    ...
    def set_prompt(self, article, examples, highlights):
        initial_prompt = ner_bottleneck_prompt.format(article=article, examples=examples, highlights_with_entities=highlights)
...
def main():
    ...
    df["highlights"] = df["highlights"].apply(lambda x: literal_eval(x))
    ...
    for idx, row in df.iterrows(): 
        ...
        highlights_prompt = build_highlight_prompt(highlights)
        model.set_prompt(article, examples, highlights_prompt)
        print(model.prompt)
        # model.run_inference()
        ...
        # gc.collect()
        #     
        # df.to_csv(f"{folder}/DACSA_test_ner_bottleneck_few_shot_{args.shot}_{model_name}_{quantization}_results.csv", index=False)
